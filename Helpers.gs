...
  const pMatch = note.match(/P-([0-9]{1,2}\/[0-9]{1,2})(?:\s*\(([^)]+)\))?/i);
  const pDate = pMatch ? normalizeString_(pMatch[1]) : "";
  const pItem = pMatch && pMatch[2] ? normalizeString_(pMatch[2]) : "";
...
  return { endDate, pDate, pItem, csp, custom };
}
...
//==============================================================
// EXTERNAL LOG (Update Change Log) - TOP INSERT + WRAP + severity + runId
//==============================================================
...
    const shortage = shortageData.get(dataKey);
    const pStr = shortage
      ? `P-${Utilities.formatDate(shortage.date, Session.getScriptTimeZone(), "M/dd")} (${shortage.item})`
      : "";
...
    // Log changes only when something changed
...
        const oldP = oldParts.pDate ? `P-${oldParts.pDate}${oldParts.pItem ? ` (${oldParts.pItem})` : ""}` : "none";
        const newP = newParts.pDate ? `P-${newParts.pDate}${newParts.pItem ? ` (${newParts.pItem})` : ""}` : "none";
...
    if (key && date) map.set(key, { date, item: r[h["Item"]] });
  });
...
  // NOTE: includes Cust PO between Product Class and Job End Date
  const headers = [
...
  // Trim trailing empty columns but preserve shape
...
  // Force Item column to text if present
  const h = getHeaders_(sheet);
  if (h["Item"] !== undefined) {
...
  // allow Suffix/Job Suffix variation where applicable
  const out = [];
...
      jobOrder: key,
      customer: "SYTELINE NOT TRACKED",
      po: custPo || "", // ✅ populate Update Change Log column F
      qty: "",
      itemNo: item,
      severity: "WARN",
      runId,
